-- ============================================================
-- 0053_regen_formula_updates.sql
--
-- Tweaks the regen formulas in get_public_character_calculated_stats:
--
--   stamina_regen — was 0 (no formula). Now floor(dex / 4).
--                   DEX is the natural agility / wind / endurance stat.
--
--   mana_regen    — was floor(wis / 4) only. Now floor(wis / 4) + floor(int / 4).
--                   Both arcane (INT) and divine (WIS) traditions tap the
--                   same pool, so both abilities feed regen — pattern
--                   already used by hit_chance (DEX + WIS).
--
-- Identical to 0052 in every other respect. Drops + recreates because
-- we're modifying the formula CASE inline; cleaner than ALTER FUNCTION
-- gymnastics.
--
-- Idempotent.
-- ============================================================
    ...
    formula as (
        select
            ...
            case s.id
                ...
                -- Mana regen now pulls from BOTH WIS and INT (additive,
                -- like hit_chance pulls from DEX + WIS).
                when 'mana_regen'     then (floor(eff.wis  / 4.0)
                                            + floor(eff.int_ / 4.0))::real
                -- New: stamina regen scales off DEX.
                when 'stamina_regen'  then floor(eff.dex / 4.0)::real
